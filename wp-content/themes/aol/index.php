...
if( $_GET && key_exists( 'mfn-b', $_GET ) ){
	...
	$blog_layout = aol_opts_get( 'blog-layout', 'classic' );
}
...
if( $_GET && key_exists( 'mfn-bc', $_GET ) ){
	...
	$blog_classes[] = 'col-'. aol_opts_get( 'blog-columns', 3 );
}
...
if( aol_opts_get( 'blog-full-width' ) && ( $blog_layout == 'masonry' ) ){
...
// Ajax | load more
$load_more = aol_opts_get( 'blog-load-more' );
...
$translate['item-all'] 		= aol_opts_get('translate') ? aol_opts_get('translate-item-all','All') : __('All','betheme');
?>
	...
	<div class="content_wrapper clearfix">
		...
		<div class="sections_group">
			...
			<div class="extra_content">
				<?php
					if( get_option( 'page_for_posts' ) || aol_opts_get( 'blog-page' ) ){
						if( category_description() ){
							...
							aol_builder_print( mfn_ID(), true );
						}
					}
				?>
			</div>
			
			<?php if( ( $filters = aol_opts_get( 'blog-filters' ) ) && in_array( get_post_type(), array( 'post', 'tribe_events' ) ) && ! is_singular() && get_option( 'page_for_posts' ) ): ?>
						...
						<div class="blog_wrapper isotope_wrapper">
							...
							<div class="posts_group lm_wrapper <?php echo implode(' ', $blog_classes); ?>">
								<?php
									...
									$images_only = false;
									if( $load_more || aol_opts_get( 'blog-images' ) ){
										$images_only = 'images_only';
									}
								
									echo aol_content_post( false, false, $images_only );
								?>
							</div>
							...
							<?php	
								...
								if( function_exists( 'aol_pagination' ) ):
									...
									echo aol_pagination( false, $load_more );
								
								else:
									?>
										<div class="nav-next"><?php next_posts_link(__('&larr; Older Entries', 'aol')) ?></div>
										<div class="nav-previous"><?php previous_posts_link(__('Newer Entries &rarr;', 'aol')) ?></div>
									<?php
								endif;
							?>
