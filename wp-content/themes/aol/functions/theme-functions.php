...
if( ! function_exists( 'mfn_add_image_size' ) )
{
	function mfn_add_image_size() {
		...
		// Blog & Portfolio | Dynamic sizes -------------------
			 ...
			 */
			$archivesW = aol_opts_get( 'featured-blog-portfolio-width', 960 );
			$archivesH = aol_opts_get( 'featured-blog-portfolio-height', 750 );
			
			$archivesC = aol_opts_get( 'featured-blog-portfolio-crop', 'crop' );
			 ...
			 * TODO: DEFAULT HEIGHT/PROPORTIONS - FOR VERIFICATION
			 */
			$singleW = aol_opts_get( 'featured-single-width', 1200 );
			$singleH = aol_opts_get( 'featured-single-height', 480 );
				
			$singleC = aol_opts_get( 'featured-single-crop', 'crop' );
...
if( ! function_exists( 'mfn_excerpt_length' ) )
{
	function mfn_excerpt_length( $length ) {
		return aol_opts_get( 'excerpt-length', 26 );
	}
}
...
if( ! function_exists( 'mfn_exclude_category' ) )
{
	function mfn_exclude_category($query) {
		...
		if( $exclude = trim( aol_opts_get( 'exclude-category' ) ) ){
			if( is_home() ){
	...
	function mfn_ID(){
		...
		if( ! is_404() ){
			...
			if( function_exists( 'is_woocommerce' ) && is_woocommerce() ){
				...
				$postID = aol_opts_get( 'portfolio-page' );
				...
				if( get_option( 'page_for_posts' ) ){
					...
					$postID = mfn_wpml_ID( aol_opts_get( 'blog-page' ) );
	...
	function mfn_layout_ID(){
		...
		if( mfn_ID() ){
			...
			if( is_single() && get_post_type() == 'post' ){
				...
				$layoutID = aol_opts_get( 'blog-single-layout' );
				...
				if( get_post_meta( mfn_ID(), 'mfn-post-custom-layout', true ) ){
					...
					$layoutID = aol_opts_get( 'portfolio-single-layout' );
 ...
 * Slider | Isset
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'aol_slider_isset' ) )
{
	function aol_slider_isset( $id = false ){
		
		$slider = false;
				
		// Global Slider Shortcode
		if( is_page() && aol_opts_get( 'slider-shortcode' ) ){
			return 'global';
		}
 ...
 * Slider | Get
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'aol_slider' ) )
{
	function aol_slider( $id = false ){
		
		$slider = false;
		$slider_type = aol_slider_isset( $id );
		...
		switch ($slider_type) {
			...
			case 'global':
		        $slider = '<div class="mfn-main-slider" id="mfn-global-slider">';
					$slider .= do_shortcode( aol_opts_get('slider-shortcode') );
				$slider .= '</div>';
		        break;
...
if( ! function_exists( 'mfn_is_mobile' ) )
{
	function mfn_is_mobile(){
		...
		if( aol_opts_get( 'responsive-parallax' ) ){
			$mobile = false;
		}
 ...
 * User OS
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'aol_user_os' ) )
{
	function aol_user_os(){
...
if( ! function_exists( 'mfn_parallax_plugin' ) )
{
	function mfn_parallax_plugin(){
		...
		$parallax = aol_opts_get( 'parallax' );
...
{
	function aol_pagination( $query = false, $load_more = false ){
		global $wp_query;	
		...
		$translate['prev'] = aol_opts_get('translate') ? aol_opts_get('translate-prev','&lsaquo; Prev page') : __('Prev page','betheme');
		$translate['next'] = aol_opts_get('translate') ? aol_opts_get('translate-next','Next page &rsaquo;') : __('Next page','betheme');
		$translate['load-more'] = aol_opts_get('translate') ? aol_opts_get('translate-load-more','Load more') : __('Load more','betheme');
		...
		$end_size = 1;
		$mid_size = 2;
		$show_all = aol_opts_get('pagination-show-all');
		$dots = false;
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'aol_nosidebar' ) )
{
	function aol_nosidebar(){
...
if( ! function_exists( 'mfn_breadcrumbs' ) )
{
	function mfn_breadcrumbs( $class = false ){
		...
		$translate['home'] 			= aol_opts_get('translate') ? aol_opts_get('translate-home','Home') : __('Home','betheme');
		...
		if( get_post_type() == 'post' ){
			...
			if( get_option( 'page_for_posts' ) ){
				...
				$blogID = aol_opts_get( 'blog-page' );		// Theme Options / Getting Started / Blog
			}
		...
		// Single -----------------------------------------
			...
			if( get_post_type() != 'post' ){
				...
				$portfolio_page_id 	= mfn_wpml_ID( aol_opts_get('portfolio-page') );
			
				// Portfolio Page ------------
				if( $slug['slug'] == aol_opts_get( 'portfolio-slug', 'portfolio-item' ) && $portfolio_page_id ){
		...
		// Taxonomy ---------------------------------------
			...
			// Portfolio ------------------------
			$post_type = get_post_type_object( get_post_type() );
			if( $post_type->name == 'portfolio' && $portfolio_page_id = mfn_wpml_ID( aol_opts_get('portfolio-page') ) ) {
				$breadcrumbs[] = '<a href="' . get_page_link( $portfolio_page_id ) . '">'. get_the_title( $portfolio_page_id ) .'</a>';
			}
	...
	function mfn_post_thumbnail( $postID, $type = false, $style = false, $images_only = false ){
		...
		if( is_single( $postID ) ){
			...
			if( get_post_type() == 'post' ){
				...
				if( ! aol_opts_get( 'blog-single-zoom' ) ){
					$link_before = '';
					$link_after = '';
				}
				...
				if( aol_opts_get( 'mfn-seo-schema-type' ) ){
			...
			$external = aol_opts_get( 'portfolio-external' );
			...
			// Hover
			if( aol_opts_get( 'portfolio-hover-title' ) ){
...
if( ! function_exists( 'mfn_get_portfolio_order' ) )
{
	function mfn_get_portfolio_order(){
		return aol_opts_get( 'portfolio-order', 'DESC' );
	}
}
...
if( ! function_exists( 'mfn_get_portfolio_orderby' ) )
{
	function mfn_get_portfolio_orderby(){
		$orderby = aol_opts_get( 'portfolio-orderby', 'date' );
		switch( $orderby ){
			case 'title':
				$orderby = 'post_title';
				break;
...
// filters ----------
if( ! function_exists( 'aol_post_navigation_sort' ) )
{
	function aol_post_navigation_sort(){
...
if( ! function_exists( 'mfn_post_navigation_header' ) )
{
	function mfn_post_navigation_header( $post_prev, $post_next, $post_home, $translate = array() ){
		...
		$style = aol_opts_get( 'prev-next-style' );
...
if( ! function_exists( 'mfn_post_navigation_sticky' ) )
{
	function mfn_post_navigation_sticky( $post, $next_prev, $icon ){
		...
		if( is_object( $post ) ){
			...
			$style = aol_opts_get( 'prev-next-sticky-style', 'default' );
...
if( ! function_exists( 'mfn_option_posts_per_page' ) )
{
	function mfn_option_posts_per_page( $value ) {
		if ( is_tax( 'portfolio-types' ) ) {
	        $posts_per_page = aol_opts_get( 'portfolio-posts', 6, true );
	    } else {
	        $posts_per_page = aol_opts_get( 'blog-posts', 5, true );
	    }
...
if( ! function_exists( 'mfn_comments_number' ) )
{
	function mfn_comments_number() {
		$translate['comment'] = aol_opts_get('translate') ? aol_opts_get('translate-comment','comment') : __('comment','betheme');
		$translate['comments'] = aol_opts_get('translate') ? aol_opts_get('translate-comments','comments') : __('comments','betheme');
		$translate['comments-off'] = aol_opts_get('translate') ? aol_opts_get('translate-comments-off','comments off') : __('comments off','betheme');
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'aol_tag_schema' ) )
{
	function aol_tag_schema(){
		...
		if( aol_opts_get( 'mfn-seo-schema-type' ) && $type ){
