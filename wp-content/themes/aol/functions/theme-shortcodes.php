...
if( ! function_exists( 'sc_blog' ) )
{
	function sc_blog( $attr, $content = null )
	{
		...
		$translate['filter'] 		= aol_opts_get('translate') ? aol_opts_get('translate-filter','Filter by') : __('Filter by','betheme');
		$translate['tags'] 			= aol_opts_get('translate') ? aol_opts_get('translate-tags','Tags') : __('Tags','betheme');
		$translate['authors'] 		= aol_opts_get('translate') ? aol_opts_get('translate-authors','Authors') : __('Authors','betheme');
		$translate['all'] 			= aol_opts_get('translate') ? aol_opts_get('translate-item-all','All') : __('All','betheme');
		...
		$output = '<div class="column_filters">';
			...
			$output .= '<div class="blog_wrapper isotope_wrapper clearfix">';
				...
				$output .= '<div class="posts_group lm_wrapper col-'. $columns .' '. $classes .'">';
					...
					$output .= aol_content_post( $query_blog, $style ,$images_only );
										
				$output .= '</div>';
	
				if( $pagination ) $output .= aol_pagination( $query_blog, $load_more );
							...
							$output .= '<div class="desc">';
								...
								if( aol_opts_get( 'blog-meta' ) ){
									$output .= '<div class="post-meta clearfix">';
										...
										if( comments_open() && aol_opts_get( 'blog-comments' ) ){
				...
				while ( $query_shop->have_posts() ){
					...
					$output .= '<li class="'. implode(' ',get_post_class()).'">';
						$output .= '<div class="item_wrapper">';
							...
							if( aol_opts_get( 'shop-images' ) == 'secondary') {
...
if( ! function_exists( 'sc_chart' ) )
{
	function sc_chart( $attr, $content = null )
	{
		...
		if( $_GET && key_exists( 'mfn-c', $_GET ) ){
			...
			$color =  aol_opts_get( 'color-counter', '#2991D6' );
		}
...
if( ! function_exists( 'sc_counter' ) )
{
	function sc_counter( $attr, $content = null )
	{
		...
		$animate_math = aol_opts_get('math-animations-disable') ? false : 'animate-math';
...
if( ! function_exists( 'sc_button' ) )
{
	function sc_button( $attr, $content = null )
	{
		...
		if( $color ){
			if( strpos($color, '#') === 0 ){
				if( aol_opts_get('button-style') == 'stroke' ){
					// Stroke | Border
	...
	function sc_portfolio( $attr, $content = null )
	{
		...
		$output = '<div class="column_filters">';
			...
			$output .= '<div class="portfolio_wrapper isotope_wrapper '. $class .'">';
				...
				if( $pagination ) $output .= aol_pagination( $query_portfolio, $load_more );
...
if( ! function_exists( 'sc_map' ) )
{
	function sc_map( $attr, $content = null )
	{
		...
		if( $api_key = trim( aol_opts_get( 'google-maps-api-key' ) ) ){
			$api_key = '?key='. $api_key;
		}
...
if( ! aol_opts_get( 'sc-gallery-disable' ) ){
