...
if( ! function_exists( 'mfn_title' ) )
{
	...
	{
		if( aol_opts_get( 'mfn-seo' ) && mfn_ID() ){
...
if( ! function_exists( 'mfn_seo' ) )
{
	function mfn_seo() 
	{
		if( aol_opts_get('mfn-seo') ){
			...
			} elseif( aol_opts_get('meta-description') ){
				echo '<meta name="description" content="'. stripslashes( aol_opts_get('meta-description') ) .'" />'."\n";
			}
			...
			} elseif( aol_opts_get('meta-keywords') ){
				echo '<meta name="keywords" content="'. stripslashes( aol_opts_get('meta-keywords') ) .'" />'."\n";
			}
			
		}
	
		// google analytics
		if( aol_opts_get( 'google-analytics' ) ){
			aol_opts_show( 'google-analytics' );
		}
	}
}
...
if( ! function_exists( 'mfn_google_remarketing' ) )
{
	...
	{
		// google remarketing
		if( aol_opts_get( 'google-remarketing' ) ){
			aol_opts_show( 'google-remarketing' );
		}
	}
}
...
if( ! function_exists( 'mfn_fonts_selected' ) )
{
	function mfn_fonts_selected(){
		...
		$fonts['content'] 		= aol_opts_get( 'font-content', 		'Roboto' );
		$fonts['menu'] 			= aol_opts_get( 'font-menu', 			'Roboto' );
		$fonts['title'] 		= aol_opts_get( 'font-title', 			'Patua One' );
		$fonts['headings'] 		= aol_opts_get( 'font-headings', 		'Patua One' );
		$fonts['headingsSmall'] = aol_opts_get( 'font-headings-small', 	'Roboto' );
		$fonts['blockquote'] 	= aol_opts_get( 'font-blockquote', 		'Patua One' );
		$fonts['decorative'] 	= aol_opts_get( 'font-decorative', 		'Patua One' );
...
if( ! function_exists( 'mfn_styles' ) )
{
	function mfn_styles() 
	{
		...
		// rtl | demo -----
		if( $_GET && key_exists( 'aol-rtl', $_GET ) ) wp_enqueue_style( 'aol-rtl', THEME_URI .'/rtl.css', false, THEME_VERSION, 'all' );
		...
		if( aol_opts_get( 'static-css' ) && ( ! ( $_GET && key_exists( 'mfn-c', $_GET ) ) ) ){	
		...
		// subset
		$subset 		= aol_opts_get('font-subset');
		if( $subset ) $subset = '&amp;subset='. str_replace(' ', '', $subset);
		
		// style & weight
		if( $weight = aol_opts_get('font-weight') ){
...
if( ! function_exists( 'mfn_styles_custom_font' ) )
{
	...
	{
		$font_custom = aol_opts_get( 'font-custom' );
		$font_custom2 = aol_opts_get( 'font-custom2' );
		...
		if( $font_custom ){
			...
			echo '<style id="mfn-dnmc-font-css">'."\n";
				echo '@font-face {';
					...
					echo 'src: url("'. aol_opts_get('font-custom-eot') .'");';
					echo 'src: url("'. aol_opts_get('font-custom-eot') .'#iefix") format("embedded-opentype"),';
						echo 'url("'. aol_opts_get('font-custom-woff') .'") format("woff"),';
						echo 'url("'. aol_opts_get('font-custom-ttf') .'") format("truetype"),';
						echo 'url("'. aol_opts_get('font-custom-svg') .'#'. $font_custom .'") format("svg");';
		...
		if( $font_custom2 ){
			...
			echo '<style id="mfn-dnmc-font2-css">'."\n";
				echo '@font-face {';
					...
					echo 'src: url("'. aol_opts_get('font-custom2-eot') .'");';
					echo 'src: url("'. aol_opts_get('font-custom2-eot') .'#iefix") format("embedded-opentype"),';
						echo 'url("'. aol_opts_get('font-custom2-woff') .'") format("woff"),';
						echo 'url("'. aol_opts_get('font-custom2-ttf') .'") format("truetype"),';
						echo 'url("'. aol_opts_get('font-custom2-svg') .'#'. $font_custom2 .'") format("svg");';
					echo 'font-weight: normal;';
					echo 'font-style: normal;';
				echo '}'."\n";
			echo '</style>'."\n";
...
if( ! function_exists( 'mfn_styles_background' ) )
{
	function mfn_styles_background()
	{
		...
		if( $layoutID = mfn_layout_ID() ){
			...
			$htmlB = aol_opts_get( 'img-page-bg' );
			$htmlP = aol_opts_get( 'position-page-bg' );
		}
		...
		if( $htmlB ){
			...
			if( $htmlP ){
				...
				} elseif( aol_opts_get( 'size-page-bg' ) ){
					if( in_array( aol_opts_get( 'size-page-bg' ), array( 'cover', 'contain' ) ) ){
						$aBg[] = 'background-size:'. aol_opts_get( 'size-page-bg' );
					} elseif( aol_opts_get( 'size-page-bg' ) == 'cover-ultrawide' ) {
		...
		if( aol_opts_get( 'img-subheader-bg' ) ){
			$headerB = aol_opts_get( 'img-subheader-bg' );
		}
		...
		$headerP = aol_opts_get( 'img-subheader-attachment' );
		...
		if( $headerB ){
			...
			if( $headerP == "fixed" ){
				...
				if( $background_attr[3] ){
					$aBg[] = 'background-size:'. $background_attr[3];
				} elseif( aol_opts_get( 'size-subheader-bg' ) ){
					if( in_array( aol_opts_get( 'size-subheader-bg' ), array( 'cover', 'contain' ) ) ){
						$aBg[] = 'background-size:'. aol_opts_get( 'size-subheader-bg' );
					} elseif( aol_opts_get( 'size-subheader-bg' ) == 'cover-ultrawide' ) {
		...
		if( get_post_meta( mfn_ID(), 'mfn-post-subheader-image', true ) ){
			...
			$subheaderB = aol_opts_get( 'subheader-image' );
		}
		
		$subheaderP = aol_opts_get( 'subheader-position' );
		...
		if( $subheaderB ){
			...
			if( $subheaderP ){
				...
				if( $background_attr[3] ){
					$aBg[] = 'background-size:'. $background_attr[3];
				} elseif( aol_opts_get( 'subheader-size' ) ){
					if( in_array( aol_opts_get( 'subheader-size' ), array( 'cover', 'contain' ) ) ){
						$aBg[] = 'background-size:'. aol_opts_get( 'subheader-size' );
					} elseif( aol_opts_get( 'subheader-size' ) == 'cover-ultrawide' ) {
		...
		$footerB = aol_opts_get( 'footer-bg-img' );
		$footerP = aol_opts_get( 'footer-bg-img-position' );
		...
		if( $footerB ){
			...
			if( $footerP ){
				...
				if( $background_attr[3] ){
					$aBg[] = 'background-size:'. $background_attr[3];
				} elseif( aol_opts_get( 'footer-bg-img-size' ) ){
					if( in_array( aol_opts_get( 'footer-bg-img-size' ), array( 'cover', 'contain' ) ) ){
						$aBg[] = 'background-size:'. aol_opts_get( 'footer-bg-img-size' );
					} elseif( aol_opts_get( 'footer-bg-img-size' ) == 'cover-ultrawide' ) {
...
if( ! function_exists( 'mfn_styles_custom' ) )
{
	...
	{	
		...
		if( $custom_css = aol_opts_get( 'custom-css' ) ){
...
if( ! function_exists( 'mfn_scripts' ) )
{
	function mfn_scripts() 
	{
		if( ! is_admin() ){
			...
			if( aol_opts_get( 'nice-scroll' ) == 'smooth' ){
...
if( ! function_exists( 'mfn_scripts_custom' ) )
{
	...
	{
		if( $custom_js = aol_opts_get( 'custom-js' ) ){
		...
		echo '<script id="mfn-dnmc-config-js">'."\n";
			echo '//<![CDATA['."\n";
				...
				echo 'window.mfn = {';
					...
					echo 'mobile_init:'. aol_opts_get( 'mobile-menu-initial', 1240 ) .',';
				
					// nice scroll
					echo 'nicescroll:'. aol_opts_get( 'nice-scroll-speed', 40 ) .',';
				
					// parallax
					echo 'parallax:"'. mfn_parallax_plugin() .'",';
					
					// responsive
					echo 'responsive:'. intval( aol_opts_get( 'responsive', 0 ) ) .',';
				
					// retina disable
					echo 'retina_js:'. intval( aol_opts_get( 'retina-js' ) ) .'';
					
				echo '};'."\n";
				
				// lightbox
				$aPrettyOptions = aol_opts_get( 'prettyphoto-options' );
				...
				echo 'window.mfn_sliders = {';
					echo 'blog:'. intval( aol_opts_get('slider-blog-timeout',0) ) .',';
					echo 'clients:'. intval( aol_opts_get('slider-clients-timeout',0) ) .',';
					echo 'offer:'. intval( aol_opts_get('slider-offer-timeout',0) ) .',';
					echo 'portfolio:'. intval( aol_opts_get('slider-portfolio-timeout',0) ) .',';
					echo 'shop:'. intval( aol_opts_get('slider-shop-timeout',0) ) .',';
					echo 'slider:'. intval( aol_opts_get('slider-slider-timeout',0) ) .',';
					echo 'testimonials:'. intval( aol_opts_get('slider-testimonials-timeout',0) );
				echo '};'."\n";
...
// header style ---------------------------------
if( ! function_exists( 'aol_header_style' ) )
{
	function aol_header_style( $firstPartOnly = false ){
		$header_layout = false;
		...
		} elseif( aol_opts_get('header-style') ){
			$header_layout =  aol_opts_get('header-style');
		}
...
// sidebar classes ------------------------------
if( ! function_exists( 'aol_sidebar_classes' ) )
{
	function aol_sidebar_classes( $has_both = false )
	{
		$classes = $both = false;
		
		if( mfn_ID() ){
			
			if( get_post_type() == 'page' && aol_opts_get('single-page-layout') ){
				
				// Theme Options | Single - Page
				$layout = aol_opts_get('single-page-layout');
										
			} elseif( get_post_type() == 'post' && is_single() && aol_opts_get('single-layout') ){
				
				// Theme Options | Single - Post
				$layout = aol_opts_get('single-layout');
				
			} elseif( get_post_type() == 'portfolio' && is_single() && aol_opts_get('single-portfolio-layout') ){
				
				// Theme Options | Single - Portfolio
				$layout = aol_opts_get('single-portfolio-layout');
		...
		if( function_exists( 'is_woocommerce' ) ){
			...
			if( function_exists( 'is_product' ) && is_product() && aol_opts_get( 'shop-sidebar' ) == 'shop' ){
				$classes = false;
			}
...
if( ! function_exists( 'mfn_body_classes' ) )
{
	function mfn_body_classes( $classes )
	{
		...
		if( $_GET && key_exists( 'mfn-c', $_GET ) ){
			...
			$classes[] = 'color-'. aol_opts_get('skin','custom');
		}
		...
		if( $_GET && key_exists( 'mfn-style', $_GET ) ){
			...
			$classes[] = 'style-'. aol_opts_get( 'style', 'default' );
		}
		...
		if( $_GET && key_exists( 'mfn-btn', $_GET ) ){
			...
			$classes[] = 'button-'. aol_opts_get( 'button-style', 'default' );
		}
		...
		if( $_GET && key_exists( 'mfn-box', $_GET ) ){
			...
			$classes[] = 'layout-'. aol_opts_get('layout','full-width');
		}
		...
		// Nice Scroll ----------------------------------------
		if( aol_opts_get( 'nice-scroll' ) == '1' ) $classes[] = 'nice-scroll-on';

		
		// Image Frame | Style --------------------------------
		if( $_GET && key_exists( 'mfn-if', $_GET ) ){
			$classes[] = 'if-'. esc_html( $_GET['mfn-if'] ); // demo
		} elseif( aol_opts_get('image-frame-style') ){
			$classes[] = 'if-'. aol_opts_get('image-frame-style');
		}
		
		// Image Frame | Border -------------------------------
		if( aol_opts_get('image-frame-border') ){
			$classes[] = 'if-border-'. aol_opts_get('image-frame-border');
		}
		
		// Image Frame | Caption -------------------------------
		if( aol_opts_get('image-frame-caption') ) $classes[] = 'if-caption-on';
		
		
		// Content Padding ------------------------------------
		if( aol_opts_get('content-remove-padding') ){
		...
		// Single Template ------------------------------------
		if( get_post_meta( mfn_ID(), 'aol-post-template', true ) ){
			$classes[] = 'single-template-'. get_post_meta( mfn_ID(), 'aol-post-template', true );
		}

		
		// Love -----------------------------------------------
		if( ! aol_opts_get('love') ) $classes[] = 'hide-love';
	
		
		// Table Hover ----------------------------------------
		if( aol_opts_get('table-hover') ) $classes[] = 'table-'. aol_opts_get('table-hover');
		
		
		// Contact Form 7 | Form Error ------------------------
		if( aol_opts_get( 'cf7-error' ) ) $classes[] = 'cf7p-'. aol_opts_get( 'cf7-error' );
		
		
		// Advanced | Other
		$layout_options = aol_opts_get( 'layout-options' );
		...
		$header_options = aol_opts_get( 'header-fw' ) ? aol_opts_get( 'header-fw' ) : false;
		
		
		// Header | Layout --------------------------
		$classes[] = aol_header_style();
		...
		} elseif( aol_opts_get('minimalist-header') == 'no' ) {
			$classes[] = 'minimalist-header-no';
		} elseif( aol_opts_get('minimalist-header') ) {
			$classes[] = 'minimalist-header';
		}
		...
		} elseif( aol_opts_get('sticky-header') ){
			$classes[] = 'sticky-header';
		}	
		...
		if( $_GET && key_exists( 'mfn-ss', $_GET ) ){
			...
			$classes[] = 'sticky-'. aol_opts_get( 'sticky-header-style', 'white' );
		}
		
		
		// Action Bar -------------------------------
		if( aol_opts_get('action-bar') ){
			$classes[] = 'ab-show';
		} else {
			$classes[] = 'ab-hide';
		}
		...
		if( $_GET && key_exists( 'mfn-subtr', $_GET ) ){
			...
			if( aol_opts_get( 'subheader-transparent' ) != 100 ){
		...
		if( $_GET && key_exists( 'mfn-sh', $_GET ) ){
			...
			$classes[] = 'subheader-'. aol_opts_get( 'subheader-style', 'title-left' );
		}
		...
		} elseif( aol_opts_get('menu-style') ) {
			$classes[] = 'menu-'. aol_opts_get('menu-style');
		}
				
		// Menu | Options ---------------------------
		$menu_options = aol_opts_get( 'menu-options' );
		...
		// Mega Menu | Style -----------------------------
		if( aol_opts_get( 'menu-mega-style' ) ) {
			$classes[] = 'mm-'. aol_opts_get( 'menu-mega-style' );
		}
		
		
		// Logo | Options ---------------------------
		if( aol_opts_get('logo-vertical-align') ) {
			$classes[] = 'logo-valign-'. aol_opts_get('logo-vertical-align');
		}
		
		$logo_options = aol_opts_get( 'logo-advanced' );
		...
		} elseif( aol_opts_get('footer-style') ) {
			$classes[] = 'footer-'. aol_opts_get('footer-style');
		}
		
		// footer | Copy & Social -------------------
		if( aol_opts_get( 'footer-hide' ) == 'center' ) {
		...
		if( ! aol_opts_get( 'responsive' ) ) $classes[] = 'responsive-off';
		
		if( aol_opts_get( 'responsive-boxed2fw' ) ) $classes[] = 'boxed2fw';
		if( aol_opts_get( 'no-hover' ) ) $classes[] = 'no-hover-'. aol_opts_get( 'no-hover' );
		if( aol_opts_get( 'no-section-bg' ) ) $classes[] = 'no-section-bg-'. aol_opts_get( 'no-section-bg' );
		if( aol_opts_get( 'responsive-top-bar' ) ) $classes[] = 'mobile-tb-'. aol_opts_get( 'responsive-top-bar' );
		if( aol_opts_get( 'responsive-mobile-menu' ) ) $classes[] = 'mobile-'. aol_opts_get( 'responsive-mobile-menu' );
		
		$classes[] = 'mobile-mini-'. aol_opts_get( 'responsive-header-minimal', 'mr-ll' );

		// responsive | tablet | options
		$responsive_header_mob = aol_opts_get( 'responsive-header-tablet' );
		...
		$responsive_header_mob = aol_opts_get( 'responsive-header-mobile' );
		if( is_array( $responsive_header_mob ) ){		
		...
		$transparent_options = aol_opts_get( 'transparent' );
		...
		// demo / debug =============================================
		if( $_GET && key_exists( 'aol-rtl' , $_GET ) ) $classes[] = 'rtl';
