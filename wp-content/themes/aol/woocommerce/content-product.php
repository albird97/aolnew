...
if( ! $product->is_in_stock() || aol_opts_get( 'shop-catalogue' ) || in_array( $product->get_type(), array( 'external', 'grouped', 'variable' ) ) ){
...
<li <?php post_class( $classes ); ?>>
	...
	<?php
		...
		$shop_images = aol_opts_get( 'shop-images' );
		...
		if( $shop_images == 'plugin' ){
			...
			echo '<div class="image_frame scale-with-grid product-loop-thumb" ontouchstart="this.classList.toggle(\'hover\');">';
				...
				if ( ! $product->is_in_stock() && $soldout = aol_opts_get( 'shop-soldout' ) ){
	...
	<div class="desc">
		...
		<?php
			...
			if( $_GET && key_exists('mfn-shop', $_GET) ){
				...
				$shop_layout = aol_opts_get( 'shop-layout', 'grid' );
			}
			...
			} elseif( aol_opts_get('shop-excerpt') ){
