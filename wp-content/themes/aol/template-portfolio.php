...
if( $_GET && key_exists('mfn-p', $_GET) ){
	...
	$portfolio_classes .= aol_opts_get( 'portfolio-layout', 'grid' );
}
...
if( $_GET && key_exists('mfn-pc', $_GET) ){
	...
	$portfolio_classes .= ' col-'. aol_opts_get( 'portfolio-columns', 3 );
}


if( $_GET && key_exists('mfn-pfw', $_GET) )	$section_class[] = 'full-width'; // demo
if( aol_opts_get('portfolio-full-width') )	$section_class[] = 'full-width';
...
if( $_GET && key_exists('mfn-iso', $_GET) ){
	$iso = true; // demo
} elseif(  aol_opts_get( 'portfolio-isotope' ) ) {
	$iso = true;
} else {
	$iso = false;
}
...
// Ajax | load more
$load_more = aol_opts_get('portfolio-load-more');
		...
		<div class="sections_group">
			...
			<?php else: ?>
				...
				<div class="extra_content">
					<?php aol_builder_print( mfn_ID(), true ); ?>
				</div>
				
				
				<?php if( $filters = aol_opts_get('portfolio-filters') ): ?>
							...
							<div class="portfolio_wrapper isotope_wrapper">
								...
								<?php 
									...
									$portfolio_args = array( 
										'post_type' 			=> 'portfolio',
										'posts_per_page' 		=> aol_opts_get( 'portfolio-posts', 6 ),
										'paged' 				=> $paged,
										'order' 				=> aol_opts_get( 'portfolio-order', 'DESC' ),
									...
									echo aol_pagination( $portfolio_query, $load_more );
