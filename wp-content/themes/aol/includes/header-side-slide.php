...
$menu_pos = 'right';
if( in_array( aol_opts_get( 'responsive-header-minimal' ), array( 'ml-ll','ml-lc','ml-lr' ) ) ){
	$menu_pos = 'left';
}
...
$side_class .= ' '. mfn_brightness( aol_opts_get( 'background-side-menu', '#191919' ) );

// side slide | hide
$ss_hide = aol_opts_get( 'responsive-side-slide' );
if( is_array( $ss_hide ) ){
...
// action button
if( $action_link = aol_opts_get( 'header-action-link' ) ){

	$action_options = aol_opts_get( 'header-action-target' );
	...
	echo '<a href="'. $action_link .'" class="button button_theme button_js action_button '. $action_class .'" '. $action_target .'><span class="button_label">'. aol_opts_get( 'header-action-title' ) .'</span></a>';
}
...
// WooCommerce Cart
global $woocommerce;
$show_cart = trim( aol_opts_get( 'shop-cart' ) );
...
if( has_nav_menu( 'lang-menu' ) ){
	...
	$lang_args = '';
	$lang_options = aol_opts_get( 'header-wpml-options' );
	$wmpl_flags = aol_opts_get( 'header-wpml' );
...
if( aol_opts_get( 'header-search' ) ){
	echo '<div class="search-wrapper">';
		...
		$translate[ 'search-placeholder' ] = aol_opts_get( 'translate' ) ? aol_opts_get( 'translate-search-placeholder', 'Enter your search' ) : __( 'Enter your search', 'betheme' );
	
		echo '<form id="side-form" method="get" action="'. esc_url( home_url( '/' ) ) .'">';
	
			if( aol_opts_get( 'header-search' ) == 'shop' ){
...
if( has_nav_menu( 'lang-menu' ) ){
	...
	if( count( $languages ) > 1 ){
		...
		$translate[ 'wpml-no' ] = aol_opts_get( 'translate' ) ? aol_opts_get( 'translate-wpml-no', 'No translations available for this page' ) : __( 'No translations available for this page', 'betheme' );
