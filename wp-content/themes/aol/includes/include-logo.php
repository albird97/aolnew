...
if( $logo_text = aol_opts_get( 'logo-text' ) ){
...
echo '<div class="logo'. $logo_class .'">';
	...
	$logo_height 	= intval( aol_opts_get( 'logo-height', 60 ) );
	$logo_padding 	= intval( aol_opts_get( 'logo-vertical-padding', 15 ) );

	$logo_options 	= aol_opts_get( 'logo-link', false );
	$logo_before	= '';
	$logo_after		= '';
	...
	if( $layoutID = mfn_layout_ID() ){
		...
		$logo['default']['main'] 			= aol_opts_get( 'logo-img', THEME_URI .'/images/logo/logo.png' );
		$logo['default']['sticky'] 			= aol_opts_get( 'sticky-logo-img' ) ? aol_opts_get( 'sticky-logo-img' ) : $logo['default']['main'];
		$logo['default']['mobile']  		= aol_opts_get( 'responsive-logo-img' ) ? aol_opts_get( 'responsive-logo-img' ) : $logo['default']['main'];
		$logo['default']['mobile-sticky']	= aol_opts_get( 'responsive-sticky-logo-img' ) ? aol_opts_get( 'responsive-sticky-logo-img' ) : $logo['default']['main'];
	
		$logo['retina']['main']  			= aol_opts_get( 'retina-logo-img' );
		$logo['retina']['sticky']  			= aol_opts_get( 'sticky-retina-logo-img' ) ? aol_opts_get( 'sticky-retina-logo-img' ) : $logo['retina']['main'];
		$logo['retina']['mobile']  			= aol_opts_get( 'responsive-retina-logo-img' ) ? aol_opts_get( 'responsive-retina-logo-img' ) : $logo['retina']['main'];
		$logo['retina']['mobile-sticky'] 	= aol_opts_get( 'responsive-sticky-retina-logo-img' ) ? aol_opts_get( 'responsive-sticky-retina-logo-img' ) : $logo['retina']['main'];
	}

	
	// SVG width
	
	if( $width = aol_opts_get( 'logo-width' ) ){
		$svg 	= ' svg';
		$width 	= 'width="'. $width .'"';
