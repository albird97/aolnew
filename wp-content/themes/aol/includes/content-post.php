...
if( ! function_exists('aol_content_post') ){
	function aol_content_post( $query = false, $style = false, $images_only = false ){
		global $wp_query;
		$output = '';
		...
		$translate['like'] 			= aol_opts_get('translate') ? aol_opts_get('translate-like','Do you like it?') : __('Do you like it?','betheme');
		...
		if( ! $style ){
			if( $_GET && key_exists('mfn-b', $_GET) ){
				...
				$style = aol_opts_get( 'blog-layout', 'classic' );
			}
		}
								...
								if( $show_meta ){
									$output .= '<div class="post-meta clearfix">';
										...
										$output .= '<div class="author-date">';	
											...
											if( $style == 'masonry tiles' && comments_open() && aol_opts_get( 'blog-comments' ) ){
